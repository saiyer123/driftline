...
A subscriber registered with `critical=True` (the ledger) is different: its
failure is re-raised to the publisher after the other handlers ran, so the
trading engine can stop rather than keep trading with an incomplete record.
"""
...
class CriticalHandlerError(RuntimeError):
    """A critical subscriber (the ledger) failed to process an event."""
...
class EventBus:
    ...
    def subscribe(self, event_type: str | type[Event], handler: Handler, critical: bool = False) -> None:
        key = event_type if isinstance(event_type, str) else event_type.__name__
        self._subs[key].append((handler, critical))
    ...
    async def publish(self, event: Event) -> None:
        ...
        for h, critical in handlers:
            try:
                await h(event)
            except Exception as exc:
                log.exception("handler %s failed for %s", h, event.type)
                if critical and failure is None:
                    failure = exc
        if failure is not None:
            raise CriticalHandlerError(f"critical handler failed for {event.type}: {failure}") from failure
